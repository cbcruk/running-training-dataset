...
`uniform-intervals`를 넣으면서 드러났다. 출처가 구조는 정확히 지정하고(1분 / 1분 ×
20~25회) 강도는 한 마디도 하지 않는데, 워크아웃 스키마는 앵커를 **최소 두 개** 요구한다.
그 최소치가 있는 이유는 좋다 — 앵커 하나는 앵커들이 서로 어긋난다는 사실을 감춘다. 다만
그것은 읽기가 둘 이상 존재한다고 전제하며, 여기서는 하나도 없다.

지금 한 것: 두 앵커 모두 `rough`로 두고, 값이 출처가 아니라 **구조에서 유도됐다**는 것을
각 `note`에 적었다. 같은 질로 25회를 끝낼 수 있는 강도가 상한이라는 논증은 반복 횟수만으로
성립하므로 기억에서 꺼낸 것이 아니다.

남는 문제: 그 구분이 `note`의 산문에만 있다. `confidence: rough`는 "느슨한 교차 모델 매핑"을
뜻하지 "출처에 없어서 우리가 추론했다"를 뜻하지 않는다. `fartlek`의 넓은 RPE 범위(의도된
처방)와 이 행의 넓은 RPE 범위(우리의 무지)가 같은 라벨을 달고 있다 — `provenance`와
`dose_question`이 각각 닫은 것과 **같은 모양의 구멍**이다.

- [ ] 앵커에 출처 여부를 표시할지 결정한다. 예: `stated: false`. 지금은 한 행뿐이라
      비용이 이득보다 클 수 있고, 두 번째 사례가 나오면 그때가 만들 시점이다.
...
- [x] **진짜 URL (프리렌더 + History API 라우팅).** 완료. 라우팅이 해시에서 History API로
      옮겨갔고, `scripts/prerender.ts`가 엔트리마다 HTML 파일 하나씩을 `dist/`에 쓴다.
      각자의 title, description, canonical URL, OG 태그, 그리고 마크업 안의 엔트리 내용까지.
      마크업은 개발 서버가 답하는 것과 같은 응답에서 온다 — 라우트마다 `router.fetch()`를
      부르고 본문을 파일로 쓰므로 둘이 갈라질 수 없다(ADR 0009). CI가 라우트 수를 데이터와
      대조하므로 조용한 회귀는 나갈 수 없다.
