...
- **정경 텍스트는 `tradition`으로 내려가고, 그러면 cite가 제거된다.** 방법이 자기를
  ...
  책은 다른 곳에서 워크아웃 명제에 붙어 있어 미부착으로 보이지 않았지만, `daniels` 체계
...
- **같은 규칙을 distribution 층에도 적용했고**, 이건 트리아지가 다루지 않던 부분이다.
  ...
  모델에 관한 것이지 피라미드형 세션 분포에 관한 것이 아니고, 그 모델 주장은 이제 체계의
...
`validate.ts`는 루트 근거가 `tradition` 이상을 주장하면서 `claim.proposition`이 없는 체계를
...
**자기 서술은 이제 소스 단위가 아니라 명제 단위로 판정한다.** 질문 1이 체계 층에서 정경
...
근거를 뒤섞고 있었다. `tradition`은 cite를 금지하므로, 정경 텍스트를 내리면 그 체계들이
...
- [ ] 나머지 열한 체계에는 `source`가 없다. 하나를 추가한다는 건 실제 텍스트를 손에 쥔다는
...
않았다. `source`는 있으면 렌더되고 없으면 아무 말도 하지 않으므로, 문서화된 세 체계와
그렇지 않은 열한 체계가 브라우징 중에 똑같아 보였다 — 독자가 무엇을 얼마나 믿을지 정하는
바로 그 순간에. 더 나쁜 건 하나의 빈칸이 서로 다른 두 사실을 덮고 있었다는 점이다: 아직
아무도 기록하지 않은 텍스트와, 인용할 텍스트가 아예 없는 체계.
...
**원래 3개에 대해 완료.** 행렬은 한 방향이었다(새 9개 체계가 원래 3개를 가리켰지만 반대는
...
**완료 — 새 체계들 사이의 교차 연결.** 사실에 부합하는 새-대-새 이주 경로 여섯 개를
추가해, 12개 체계 모두가 최소 2개의 인바운드 항목을 갖게 됐다:
...
대부분의 체계는 세션당 볼륨 상한을 진술하지 않고, 빈 필드가 정직하다. 문서화된 것만 추가할
것:
...
여덟 체계가 `distribution.model`은 갖지만 명시적 `zones` 분해는 없다.
...
- [ ] `system: null` 용례 행으로 한국어 통칭을 수집한다. 스키마에 슬롯이 이미 있다 — "어느
      체계에도 속하지 않는 통칭"이 그 필드의 정의다.
- [ ] `calls_it`에 한국어를 **직접 넣지 말 것.** 그 필드는 "이 체계가 이렇게 부른다"는
