...
const resolveRpe = (ref, self) => (ref === "self" ? rpe(self) : rpe(byId[ref]));
...
const flatten = (segs, self) =>
  segs.flatMap((s) =>
    s.kind === "repeat"
      ? Array.from({ length: mid(s.count) }, () => flatten(s.children, self)).flat()
      : [
          {
            kind: s.kind,
            rpe: resolveRpe(s.intensity_ref, self),
            secs: s.duration
              ? mid(s.duration)
              : mid(s.distance) / NOMINAL_MPS(resolveRpe(s.intensity_ref, self)),
          },
        ],
  );
...
function render(w) {
  const segs = flatten(w.structure.segments, w);
...
for (const w of workouts) {
  writeFileSync(resolve(root, `out/${w.id}.svg`), render(w));
