 ...
 */
export function WorkoutDetail(handle: Handle<{ workout: Workout }>) {
  return () => {
    const w = handle.props.workout

    return (
      <>
        <EntryLink className="back" to={routes.workouts.href()}>
          ← 워크아웃 목록
        </EntryLink>
 ...
 */
function FalsificationTest(handle: Handle<{ test: Workout['test'] }>) {
  return () => {
    const test = handle.props.test
    if (!test.detectable) {
      return (
        <>
          ...
          <p className="belief-note">
            관찰 불가능한 null은 해석할 수 없다. 이건 믿음이다 — 주당 몇 분을 쓸지 결정하는 문제일
            뿐.
          </p>
        </>
      )
                ...
                <div className={`confound sev-${c.severity}`} key={c.factor}>
                  <div className="confound-head">
                    ...
                    {c.shares_mechanism ? (
                      <span
                        className="shares"
                        title="acts through the same physiology as the claim"
                      >
                        같은 기전
                      </span>
                    ) : null}
 ...
 * 가리킬 수 있다.
 */
function CollisionTable(handle: Handle<{ id: string }>) {
  return () => {
    const uses = usage.filter((u) => u.workout === handle.props.id)
            ...
            {uses.map((u: Usage, i: number) => {
              ...
              return (
                <tr className={u.collides ? 'collides' : ''} key={`${u.system ?? 'none'}-${i}`}>
                  <td>
                    {u.system ? (
                      <EntryLink to={routes.system.href({ id: u.system })}>{sysName}</EntryLink>
                    ) : (
