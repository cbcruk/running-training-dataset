 ...
 * 유일한 강한 UI 규칙이기 때문이다.
 */
export function SystemDetail(handle: Handle<{ system: System }>) {
  return () => {
    const s = handle.props.system
...
function SwitchingCosts(handle: Handle<{ system: System }>) {
  return () => {
    const entries = handle.props.system.switching_cost || []
...
function IntensityDistribution(handle: Handle<{ distribution?: Distribution }>) {
  return () => {
    const distribution = handle.props.distribution
...
function Phases(handle: Handle<{ phases?: Phase[] }>) {
  return () => {
    const phases = handle.props.phases
            ...
            <div className="phase" key={p.name}>
              ...
              <div className="wchips">
                {(p.emphasis || []).map((wid: string) => (
                  <WChip key={wid} to={routes.workout.href({ id: wid })}>
...
function VolumeCaps(handle: Handle<{ caps?: VolumeCap[] }>) {
  return () => {
    const caps = handle.props.caps
...
function Caveats(handle: Handle<{ caveats?: string[] }>) {
  return () => {
    const caveats = handle.props.caveats
