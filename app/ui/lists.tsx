 ...
 *
 * 검색 결과는 여기 없다. 그건 클라이언트에서 돌고(app/client/search.tsx), 이유는
 * ADR 0009에 적혀 있다 — 정적으로 서빙되는 파일은 `?q=`를 볼 수 없다.
 */
import { Link } from '@tanstack/react-router'

import { AnchorLink, ProvenanceBadge, SystemLink, TierBadge, WorkoutLink } from './primitives.tsx'
import {
  ANCHOR_CONSTRUCTS,
  anchors,
  km,
 ...
 */
export function SystemCard({ system: s, brief = false }: { system: System; brief?: boolean }) {
  ...
  return (
    <SystemLink className="card sys-card" id={s.id}>
...
export function WorkoutCard({ workout: w, brief = false }: { workout: Workout; brief?: boolean }) {
  return (
    <WorkoutLink className="card wk-card" id={w.id}>
      <div className="card-head">
...
export function AnchorCard({ anchor: a }: { anchor: Anchor }) {
  const sys = systemsByAnchor[a.model]?.length || 0
  const wk = workoutsByAnchor[a.model]?.length || 0
  return (
    <AnchorLink className="card anchor-card" model={a.model}>
      <div className="card-head">
        <h2>
          <code>{a.model}</code>
        </h2>
...
/** `id`가 비면 무엇이 없는지 이름 붙일 것이 없다 — `/404` 라우트가 그 경우다. */
