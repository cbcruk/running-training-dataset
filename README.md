...
| `tradition` | 다들 하는데 아무도 모른다 | **금지**  | `dose_question` 필수           |
...
**`tradition`도 어떤 종류의 모름인지 말해야 한다.** 같은 낱말이 서로 다른 두 가지를 덮고 있었다: 통제 연구를 돌릴 수 있는데 아무도 돌리지 않은 것과, 지금 서술된 형태로는 "얼마나"라는 질문 자체가 성립하지 않는 것. 앞의 것은 연구가 메우는 공백이고 뒤의 것은 영구적인 성질이라, 같은 배지로 읽히면 독자가 올 리 없는 값을 기다린다. 워크아웃의 `tradition` 근거는 이제 **`dose_question`** 을 요구한다 — `open` 또는 `unaskable`.

이건 판단이 아니라 유도다. 행이 `structure.stochastic`으로 "내 구간은 예시일 뿐"이라고 스스로 선언하는 것이 곧 용량에 가리킬 대상이 없다는 뜻이므로, `rules.ts`가 양방향으로 묶는다: 확률적인 행은 `unaskable`이어야 하고, `unaskable`인 행은 확률적이어야 한다. 어느 쪽도 혼자 선언될 수 없다. 해당하는 행은 [`docs/counts.md`](docs/counts.md)가 센다. 오늘은 파틀렉뿐이고 그게 요점이다 — 파틀렉은 정의가 아직 안 된 개념이 아니라 **정의가 열려 있는 것이 정의**인 개념이고, 그래서 그 행의 공백은 채워질 수 없다. 정책 단위 시험("주 3회 감각대로 서지를 넣어라")은 여전히 가능하다. 불가능한 건 용량-반응이다.

훈련법에는 이 필드가 없다. 훈련법은 프로토콜이 아니라 내기이고, 용량 질문은 프로토콜에 묻는 것이다.
...
- `structure.stochastic`인 워크아웃에 `dose_question: open` → 위반. 구간이 예시라고 선언한 행에는 물을 용량이 없다. 역방향도 같다: `unaskable`인데 확률적이지 않으면 구조가 반대를 말하고 있다.
