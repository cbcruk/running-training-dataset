...
- **아직 아무것도 검증되지 않았다.** 모든 체계와 워크아웃이 `status: draft`다. 인용은 정규화되고 기계로 강제되지만(하나의 참조, 하나의 문자열), 그건 소스가 어디서나 같게 _읽힌다_ 는 뜻일 뿐 그것이 붙은 주장을 뒷받침한다는 뜻이 아니다. 검증 체크리스트는 [`docs/TODO.md`](docs/TODO.md#1-검증-draft--verified)에 있다.
