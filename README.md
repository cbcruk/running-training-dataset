...
**Status: skeleton.** 3 systems, 5 workouts. Every row is `status: draft` — no citation has been human-verified.
...
Current distribution: **consensus 3 / plausible 7 / tradition 12.** Tradition dominates. That is the honest shape of running knowledge; forcing the ratio the other way kills the project.
...
data/
  ...
  workouts.json      # 5  - detail view. claim / test / structure / intensity
  usage.json         # 12 - (system, workout) -> calls_it. The collision table.
